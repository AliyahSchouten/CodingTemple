...
```{r}
View(read_csv("C:/Users/Alex Lucchesi/coding-temple/coding_temple_data_analytics_ft/week-7/femaleMiceWeights.csv"))
```
...
my_data <- read.csv("C:/Users/Alex Lucchesi/coding-temple/coding_temple_data_analytics_ft/week-7/femaleMiceWeights.csv") %>% as_tibble()
my_data
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    double_char <- function(str){
      ...
      for (char in strsplit(str, "")[[1]]) {
        repeated_string <- paste0(repeated_string, char, char)
      }
    ...
    two_oldest_ages <- function(ages) {
      ...
      return(c(sorted_ages[2], sorted_ages[1]))
    }
    #Promblem 3
    ...
    square_digits <- function(num){
      ...
      for (digit in num_digits) {
        squared_str <- paste0(squared_str, as.character(as.numeric(digit)^2))
      }
      
      squared_num <- as.integer(squared_str)
      return(squared_num)
    ...
    number <- function(lines) {
      ...
      numbered_lines <- character(length(lines))
      
      for (i in seq_along(lines)) {
        numbered_lines[i] <- paste0(i, ": ", lines[i])
      }
...
2.  Create a function that checks an input of type `int` and returns
    ...
    # Test cases
    num1 <- 9
    result1 <- fizz_buzz(num1)
    cat("Input:", num1, "Output:", result1, "\n")

    num2 <- 10
    result2 <- fizz_buzz(num2)
    cat("Input:", num2, "Output:", result2, "\n")

    num3 <- 15
    result3 <- fizz_buzz(num3)
    cat("Input:", num3, "Output:", result3, "\n")

    num4 <- 7
    result4 <- fizz_buzz(num4)
    cat("Input:", num4, "Output:", result4, "\n")
    ```
...
continuous_columns <- c("mpg", "disp", "hp", "drat", "wt", "qsec")
discrete_columns <- c("cyl", "vs", "am", "gear", "carb")

# Basic summary using dplyr
summary_data <- mtcars %>%
...
ggplot(mtcars, aes(x = hp, y = mpg)) +
  geom_point() +
  labs(x = "Horsepower", y = "Miles per Gallon", title = "Scatter Plot: MPG vs. HP")

# Mutate columns to create new features
mtcars <- mtcars %>%
...
# Let's hypothesize that cars with more cylinders (higher cyl values) tend to have higher horsepower (hp) per cylinder.

# Analyze the hypothesis
hp_per_cyl_summary <- mtcars %>%
  group_by(cyl) %>%
  summarise(mean_hp_per_cyl = mean(hp_per_cyl))

print(hp_per_cyl_summary)

# Create a bar plot to visualize the hypothesis
ggplot(hp_per_cyl_summary, aes(x = factor(cyl), y = mean_hp_per_cyl)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(x = "Cylinders", y = "Mean HP per Cylinder", title = "Mean HP per Cylinder by Cylinder Count")
